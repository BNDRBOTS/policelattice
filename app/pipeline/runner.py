...
    if not source:
        source = DataSource(id=source_id, name=source_id, category="unknown", adapter="manual", access_mode="manual")
        session.add(source)
        session.commit()
    return source
...
def run_all_due() -> dict[str, int]:
    """Run all sources whose schedule is due. This is intentionally simple;
